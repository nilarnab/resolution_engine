    ...
    foreach ($premises as $premise)
{
    ...
    foreach ($temp_array as $literal)
    {
        ...
        if (strpos($literal, "<NEG>") !== false)
        {
            ...
            // we have to check if it is feasable or not
            $findable = substr($literal, 5, strlen($literal));
            for($itr = 0; $itr < count($premises); $itr++)
            {
                $search = explode("<OR>", $premises[$itr]);
                
                if (in_array($findable, $search))
                {
                    // negation is found
                    $neg_pres = true;
                    $neg_found_once = true;
            ...
            if ($neg_pres === true && $neg_found_once === true)
            {
                break;
            }
    ...
    if ($neg_pres === true)
    {
        ...
        foreach ($premises as $other)
        {
            ...
            foreach ($temp_other as $other_literal)
            {
                ...
                if (strpos($other_literal, $findable) !== false && strpos($other_literal, $literal) === false)
                {
                    // appropriate match for resolution found
                    
                    echo "APPROPRIATE RSOLUTOIN FOUND HERE\n";
...
if (($neg_found_once && $appropriate_premise_found_once) && count($premises) > 1)
{
...
<?php
$premises = ["A<OR>B<OR><NEG>C", "<NEG>A<OR>C"];
